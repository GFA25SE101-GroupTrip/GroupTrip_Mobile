...
import 'package:group_trip/features/trip/presentation/widgets/process/inprogress_body.dart';
import 'package:group_trip/features/trip/presentation/widgets/process/deposit_paid/deposit_header.dart';

class InProgressProcessScreen extends ConsumerStatefulWidget {
  final String departureId;
  final Map<String, dynamic> tripData;

  static const routeName = '/inprogress_process';
  const InProgressProcessScreen({
    super.key,
  ...
  @override
  ConsumerState<InProgressProcessScreen> createState() => _InProgressProcessScreenState();
}

class _InProgressProcessScreenState extends ConsumerState<InProgressProcessScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final String displayTitle = _tripModel?.name ?? 'Chuyến đi đang diễn ra';
  ...
  Future<void> _refreshTripData() async {
    try {
      ...
      await ref.read(mytripNotifierProvider.notifier).fetchMyTrips(status: 'InProgress');
      
      // Refetch tracking route data
      ref.refresh(mytripTrackingRouteProvider(widget.departureId));
    } catch (e) {
