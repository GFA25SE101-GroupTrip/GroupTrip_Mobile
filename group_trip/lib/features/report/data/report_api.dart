...
class ReportRemoteDataSource {
    ...
    Future<List<ReportModel>> fetchReports(String userID) async {
        final response = await apiClient.get('user','/api/report/requests/$userID');
    ...
    Future<ReportResponse> fetchReportDetail(String reportID) async {
      ...
      final response = await apiClient.get('user','/api/report/request/$reportID');
      if (response.statusCode != null &&
          response.statusCode! >= 200 &&
          response.statusCode! < 300) {
        dynamic payload = response.data;
        ...
        throw Exception('Failed to load report detail: status=${response.statusCode}');
      }
    }
